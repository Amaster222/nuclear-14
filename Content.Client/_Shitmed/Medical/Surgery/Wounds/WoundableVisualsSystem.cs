...
public sealed class WoundableVisualsSystem : VisualizerSystem<WoundableVisualsComponent>
{
    ...
    private const string LayerKeyPrefix = "ShitmedWound";
    #endregion
        ...
        string? color = null)
    {
        ...
        // Prototype RSI paths are relative to Resources/Textures, whereas this
        // direct SpriteSpecifier construction expects a full resource path.
        // Normalize both conventional and already-rooted values without adding
        // a duplicate Textures segment.
        var relativePath = sprite.TrimStart('/');
        var resourcePath = relativePath.StartsWith("Textures/", StringComparison.Ordinal)
            ? $"/{relativePath}"
    ...
    private void UpdateDamageVisuals(Entity<WoundableVisualsComponent> visuals, Entity<SpriteComponent?> sprite)
    {
        ...
        {
            if (!_sprite.LayerMapTryGet(sprite, BuildLayerKey(visuals.Comp.OccupiedLayer, group.Key), out var damageLayer, false))
                continue;
    ...
    private bool TryGetWoundData(Entity<WoundableVisualsComponent?> entity, [NotNullWhen(true)] out WoundVisualizerGroupData? wounds)
    {
        ...
        if (!Resolve(entity, ref entity.Comp)
            ...
            || !_appearance.TryGetData(entity.Owner, WoundableVisualizerKeys.Wounds, out wounds, appearance))
            return false;
    ...
    private static string BuildLayerKey(Enum baseLayer, string suffix) => $"{LayerKeyPrefix}{baseLayer}{suffix}";
    private static string BuildLayerKey(string baseLayer, string suffix) => $"{LayerKeyPrefix}{baseLayer}{suffix}";
