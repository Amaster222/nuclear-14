...
using Content.Shared._Misfits.Common.Genetics.Mutations;
...
public sealed partial class MutationSystem : CommonMutationSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<MutatableComponent, StatusEffectEndedEvent>(OnStatusEffectEnded);
    ...
    private void OnMapInit(Entity<MutatableComponent> ent, ref MapInitEvent args)
    {
        ...
        if (ent.Comp.GeneticDna.Length == 0)
            ent.Comp.GeneticDna = RandomDna(32);
    ...
    private void OnStatusEffectEnded(Entity<MutatableComponent> ent, ref StatusEffectEndedEvent args)
    {
        if (_net.IsClient || args.Key != ent.Comp.MeltingEffect || ent.Comp.TotalInstability < ent.Comp.MaxInstability)
            return;

        ClearMutations(ent.Owner, automatic: true, predicted: false);
        Scramble(ent);
        _damageable.TryChangeDamage(ent.Owner,
            new DamageSpecifier(_prototype.Index(Cellular), 200),
   ...
   public EntityUid? GetMutationTarget(EntityUid uid)
    ...
    {
        if (_net.IsClient || !_mutatableQuery.TryComp(uid, out var comp))
            return;

        var dna = comp.GeneticDna;
    ...
    public string? GetDna(EntityUid uid)
        => _mutatableQuery.CompOrNull(uid)?.GeneticDna;

    public void SetDna(Entity<MutatableComponent?> ent, string dna)
    {
        if (!_mutatableQuery.Resolve(ent, ref ent.Comp, false) || ent.Comp.GeneticDna == dna)
            return;

        ent.Comp.GeneticDna = dna;
        Dirty(ent, ent.Comp);
    }
    ...
    private string RandomDna(int length)
    {
        ...
        for (var i = 0; i < length; i++)
            builder.Append(_random.Pick(MutationData.ATGC));
    ...
    public void AddInstability(Entity<MutatableComponent> ent, int instability, bool automatic, bool predicted)
    {
        ...
        if (ent.Comp.TotalInstability >= ent.Comp.MaxInstability)
            _status.TryAddStatusEffect(ent.Owner, ent.Comp.MeltingEffect, ent.Comp.MeltDuration, true);
        else
    ...
    public Entity<MutationComponent>? GetActionMutation(EntityUid uid)
    {
        ...
        if (!_actions.ResolveActionData(uid, ref action) || action.Container is not {} mutation)
            return null;
