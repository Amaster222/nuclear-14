...
public sealed class TourniquetSystem : EntitySystem
{
    ...
    private void OnBodyDoAfter(EntityUid ent, BodyComponent comp, ref TourniquetDoAfterEvent args)
    {
        ...
        if (targetPart == null)
        {
            ...
            foreach (var woundEnt in tourniquetableWounds)
            {
                ...
                SetWoundBleeding(woundEnt, false, bleedInflicter);
        ...
        else
        {
            ...
            SetWoundableBleeding(targetPart.Value.Id, false);
            ...
            foreach (var woundable in _wound.GetAllWoundableChildren(targetPart.Value.Id))
            {
                ...
                SetWoundableBleeding(woundable, false);
            }
    ...
    private void OnTourniquetTakenOff(Entity<BodyComponent> ent, ref RemoveTourniquetDoAfterEvent args)
    {
        ...
        if (tourniquet.BlockedBodyParts.Contains(bodyPartComp.PartType))
        {
            foreach (var woundEnt in _wound.GetWoundableWounds(tourniquetedBodyPart.Value))
            {
                ...
                SetWoundBleeding(woundEnt, true, bleedInflicter);
            }
        }
        else
        {
            _pain.TryRemovePainFeelsModifier(args.Used.Value, "Tourniquet", tourniquetedBodyPart.Value);
            SetWoundableBleeding(tourniquetedBodyPart.Value, true);
            ...
            foreach (var woundable in _wound.GetAllWoundableChildren(tourniquetedBodyPart.Value))
            {
                ...
                SetWoundableBleeding(woundable, true);
            }
        }
    ...
    private void SetWoundableBleeding(EntityUid woundable, bool canBleed)
    {
        foreach (var wound in _wound.GetWoundableWounds(woundable))
        {
            if (TryComp<BleedInflicterComponent>(wound, out var bleeds))
                SetWoundBleeding(wound, canBleed, bleeds);
        }
    }

    private void SetWoundBleeding(EntityUid wound, bool canBleed, BleedInflicterComponent bleeds)
    {
        if (canBleed)
        {
            bleeds.BleedingModifiers.Remove("TourniquetPresent");
            bleeds.IsBleeding = bleeds.BleedingAmountRaw > 0;
        }
        else
        {
            bleeds.BleedingModifiers["TourniquetPresent"] = (Priority: 100, CanBleed: false);
            bleeds.IsBleeding = false;
        }

        // The active bloodstream implementation observes this replicated wound
        // state; Dirty also keeps the analyzer and limb visuals in sync.
        Dirty(wound, bleeds);
    }
}
