...
public sealed partial class VertibirdSystem
{
    ...
    /// <summary>
    /// On the ground the crate comes out wherever the person hauling it is standing, since
    /// dropping it at the aircraft's own coordinates would bury it under the hull. Airborne
    /// that is not an option: the one unloading is strapped in, so their coordinates are the
    /// craft's, and a crate spawned inside the hull shoves the whole aircraft around. Those
    /// go straight down to ground level, directly beneath the craft.
    /// </summary>
    private void DropCargo(Entity<VertibirdComponent> ent, EntityUid crate, BaseContainer container, EntityUid user)
    {
        ...
        {
            _container.Remove(crate, container, destination: Transform(user).Coordinates);
            return;
        }
